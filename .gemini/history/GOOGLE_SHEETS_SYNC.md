...
- `source_id` - Уникальный идентификатор из Google Sheets (например, GS-1, GS-2) - **ОБЯЗАТЕЛЬНО**
...
1. **Проверка существования вопроса**: Система проверяет, существует ли вопрос с таким `source_id` в базе
2. **Если вопрос новый** (`source_id` не найден):
   - Добавляется новый вопрос
   - Добавляются варианты ответов
   - Добавляются теги
3. **Если вопрос существует** (`source_id` найден):
   - Обновляются все поля вопроса
   - Удаляются старые варианты ответов и теги
   - Добавляются новые варианты ответов и теги
4. **Прогресс пользователя сохраняется**: Вопросы не удаляются, только обновляются, поэтому `user_progress` не теряется

### Важно:
- **source_id должен быть уникальным** для каждого вопроса
- **source_id не должен изменяться** после создания (иначе вопрос будет создан заново)
- Используйте формулу `="GS-" & ROW()-1` в Google Sheets для автоматической генерации
...
2. **ВАЖНО:** В первой колонке (`source_id`) используйте формулу `="GS-" & ROW()-1` для автоматической генерации уникального ID
3. Добавьте новую строку:
   ```
   source_id: GS-1 (автоматически через формулу)
   topic_number: 1
   difficulty: medium
   is_premium: ✗
   type: single
...
1. Найдите вопрос в Google Sheets по `source_id`
2. Измените нужные поля (текст вопроса, варианты ответов, объяснение и т.д.)
3. Запустите синхронизацию
4. Вопрос будет обновлен в базе данных, **прогресс пользователя сохранится**

---

## 🎯 Советы по работе

1. **source_id обязателен** - используйте формулу `="GS-" & ROW()-1` в первой колонке для автоматической генерации
2. **Не изменяйте source_id** после создания вопроса - это приведет к созданию дубликата
3. **Используйте символы ✔ и ✗** вместо TRUE/FALSE - так нагляднее
4. **Теги пишите на английском** (с маленькой буквы), через запятую без пробелов
5. **Обязательные поля:** source_id, question_ru/es/en, хотя бы 2 варианта ответа
6. **Изображения:** загружайте в Supabase Storage или используйте CDN, вставляйте URL
7. **Для вопросов с несколькими правильными ответами** установите `type: multiple` и отметьте несколько `is_correct_X` как TRUE
8. **При обновлении вопроса** старые варианты ответов и теги автоматически удаляются и заменяются новыми
...
2. Проверьте, что `source_id` заполнен и уникален
3. Проверьте, что `topic_number` существует в таблице Topics
4. Проверьте обязательные поля: source_id, question_ru, question_es, question_en

### Вопросы дублируются при синхронизации
1. Проверьте, что `source_id` уникален для каждого вопроса
2. Убедитесь, что не изменили `source_id` существующего вопроса
3. Проверьте, что формула `="GS-" & ROW()-1` работает корректно
