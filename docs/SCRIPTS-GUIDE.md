...
**Особенности:**
- Использует модель `gemini-3.1-pro-preview` (основная модель)
...
### Модели Gemini:
- **`gemini-3.1-pro-preview`** — основная самая умная модель (используем по умолчанию)
- **`gemini-2.0-flash`** — быстрая альтернатива (если 3.1 перегружена)

### Где менять модель:
В файле `scripts/enrich-batch-v2.js` или `scripts/fix-all-issues.js`:
```javascript
model: "gemini-3.1-pro-preview", // <- здесь
```
...
**Решение:** Поменяй модель на `gemini-2.0-flash` или подожди 5 минут
