      ...
      <table className="w-full min-w-[480px] text-sm">
        <thead>
          <tr className="border-b border-zinc-200 dark:border-white/5 bg-zinc-50 dark:bg-white/[0.03]">
            {headers.map((h, i) => (
              <th key={i} className="px-5 py-3 text-left text-[11px] font-bold uppercase tracking-widest text-zinc-500">
                {h}
              </th>
            ))}
          </tr>
        ...
        <tbody className="divide-y divide-zinc-200 dark:divide-white/5">
          {rows.map((row, ri) => (
            <tr key={ri} className="hover:bg-zinc-50 dark:hover:bg-white/[0.02] transition-colors">
              {row.map((cell, ci) => (
                <td key={ci} className={cn("px-5 py-3.5 text-zinc-700 dark:text-zinc-300 leading-snug align-top", ci === 0 && "font-medium text-zinc-900 dark:text-white")}>
                  {cell}
                </td>
              ))}
            </tr>
...
export function ArticleCardGrid({ cards, cols = 2 }: ArticleCardGridProps) {
  return (
    <div className={cn("my-8 not-prose grid gap-4", cols === 3 ? "grid-cols-1 sm:grid-cols-3" : "grid-cols-1 sm:grid-cols-2")}>
      {cards.map((card, i) => (
        <div key={i} className="rounded-2xl bg-zinc-50 dark:bg-[#0c1523] border border-zinc-200 dark:border-transparent p-5 hover:bg-zinc-100 dark:hover:bg-[#0f1a2b] transition-all">
      ...
      <table className="w-full min-w-[400px] text-sm">
        <thead>
          <tr className="border-b border-zinc-200 dark:border-white/5">
            <th className="px-5 py-4 text-left text-xs font-bold uppercase tracking-widest text-zinc-500 dark:text-zinc-600 w-1/2">Параметр</th>
            <th className="px-5 py-4 text-center text-xs font-bold uppercase tracking-widest text-zinc-500">{headerA}</th>
            <th className="px-5 py-4 text-center text-xs font-bold uppercase tracking-widest text-blue-600 dark:text-blue-400 bg-blue-500/5">{headerB}</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-zinc-200 dark:divide-white/5">
          {rows.map((row, i) => (
            <tr key={i} className="hover:bg-zinc-50 dark:hover:bg-white/[0.015] transition-colors">
              ...
              <td className="px-5 py-3.5 text-center"><ComparisonCell value={row.a} /></td>
              <td className="px-5 py-3.5 text-center bg-blue-500/[0.03]"><ComparisonCell value={row.b} /></td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div className="my-10 not-prose relative rounded-3xl border border-blue-500/20 bg-gradient-to-br from-blue-500/10 via-blue-500/5 to-transparent overflow-hidden">
      ...
      <div className="relative z-10 p-7 md:p-10">
        ...
        <h3 className="text-2xl md:text-3xl font-black text-zinc-900 dark:text-white leading-tight mb-2">{title}</h3>
        <p className="text-zinc-600 dark:text-zinc-400 text-sm leading-relaxed mb-5 max-w-md">{description}</p>
        <div className="flex flex-wrap gap-3">
          <a href={primaryHref} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 px-7 py-3.5 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm transition-all hover:shadow-[0_0_25px_rgba(59,130,246,0.4)] no-underline">
            {primaryLabel} <ArrowRight className="w-4 h-4" />
          </a>
          <a href={secondaryHref} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 px-7 py-3 rounded-2xl border border-zinc-300 dark:border-white/10 hover:bg-zinc-100 dark:hover:bg-white/5 text-zinc-700 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white font-semibold text-sm transition-colors no-underline">
            {secondaryLabel}
          </a>
        </div>
        <div className="mt-6 pt-5 border-t border-zinc-200 dark:border-white/6 flex items-center gap-2">
          <Zap className="w-3.5 h-3.5 text-emerald-500" />
          <span className="text-xs text-zinc-500">9 из 10 наших студентов сдают теорию DGT с первой попытки</span>
        </div>
      </div>
    </div>
  );
