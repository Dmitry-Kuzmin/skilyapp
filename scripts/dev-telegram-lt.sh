...
# Скрипт для автоматического запуска dev сервера и Localtunnel
# Использование: ./scripts/dev-telegram-lt.sh
...
echo "🚀 Запуск локальной разработки для Telegram Web App (Localtunnel)"
echo "=================================================================="
echo ""
...
# Используем npx для запуска localtunnel
...
echo "⏳ Ожидание получения Localtunnel URL..."
TUNNEL_URL=""
for i in {1..15}; do
    sleep 2
...
if [ -z "$TUNNEL_URL" ]; then
    ...
    echo "   ./scripts/start-localtunnel.sh"
    echo ""
    echo "Или проверьте URL вручную в выводе localtunnel выше"
fi
...
if [ -n "$TUNNEL_URL" ]; then
    ...
    echo "💡 Найдите URL в выводе localtunnel выше (строка с https://...loca.lt)"
...
# Показываем логи localtunnel в реальном времени
tail -f "$TUNNEL_LOG" &
TAIL_PID=$!
