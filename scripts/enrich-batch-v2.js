...
async function enrichQuestion(q) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            const prompt = `
Ты — Senior Driving Instructor (эксперт по ПДД Испании DGT). Твоя задача: обогатить тестовый вопрос профессиональными объяснениями.
...
Используй только один эмодзи-маркер 🎓. Сразу после смайлика идет текст.
Формат:
...
ВАЖНО: НИКАКИХ СЛОВ "Правило", "Сравнение", "Шпаргалка", "Запоминалка" перед текстом. Только 🎓 в начале.
