...
if (!SUPABASE_URL || !SUPABASE_KEY) {
  console.error("❌ Ошибка: Не найдены переменные VITE_SUPABASE_URL или VITE_SUPABASE_ANON_KEY");
  process.exit(1);
}
 ...
 */
function parseCorrectAnswer(correctString: string): string {
 ...
 */
async function importQuestions(category: string, filename: string): Promise<number> {
