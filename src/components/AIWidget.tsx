...
${imageUrl ? `\n⚠️ КРИТИЧНО: ЕСТЬ ИЗОБРАЖЕНИЕ К ВОПРОСУ:\n${interfaceLanguage === 'ru' ? 'На изображении показана дорожная ситуация/знак, обязательно проанализируй её и упомяни детали (дорожные знаки, разметку, ситуацию) в объяснении. URL для справки' : interfaceLanguage === 'en' ? 'The image shows a road situation/sign. MUST analyze it and mention details (road signs, markings, situation) in the explanation. URL for reference' : 'La imagen muestra una situación/señal de carretera. DEBES analizarla y mencionar detalles (señales, marcas, situación) en la explicación. URL de referencia'}: ${imageUrl}` : ''}
`;
