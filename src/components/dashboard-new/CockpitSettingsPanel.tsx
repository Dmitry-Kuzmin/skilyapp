...
const Section: React.FC<{ 
  ...
  return (
    <div className={`rounded-2xl border ${sectionBgClass} p-4 backdrop-blur-sm relative overflow-hidden`}>
      ...
      <div className="relative z-10 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            {description && <p className={`text-[11px] ${descClass}`}>{description}</p>}
          </div>
        </div>
        <div>{children}</div>
      </div>
    ...
    <div className={`relative w-full h-full ${containerBgClass} overflow-hidden`}>
      ...
      <div className="relative z-10 p-6 h-full flex flex-col">
        <div className="flex items-center justify-between mb-6">
          <div>
            <p className={`text-[10px] uppercase tracking-[0.4em] ${textSecondaryClass}`}>Cockpit</p>
            <h3 className={`text-xl font-semibold ${textPrimaryClass} mt-1`}>Настройки пилота</h3>
          </div>
          <div className={`px-3 py-1 rounded-full border ${isDarkTheme ? 'border-white/10 text-white/80' : 'border-slate-200/60 text-slate-700'} text-[11px] flex items-center gap-2`}>
            <Gauge className={`w-4 h-4 ${isDarkTheme ? 'text-emerald-300' : 'text-emerald-600'}`} />
            Custom
          </div>
        </div>

        <div className="grid gap-4 flex-1 overflow-y-auto">
        <Section title="Язык" icon={<Languages className={`w-4 h-4 ${isDarkTheme ? 'text-cyan-300' : 'text-cyan-600'}`} />} description="Интерфейс и обучение" isDarkTheme={isDarkTheme}>
        ...
        <Section title="Тема" icon={<Palette className={`w-4 h-4 ${isDarkTheme ? 'text-indigo-300' : 'text-indigo-600'}`} />} description="Внешний вид тарелки" isDarkTheme={isDarkTheme}>
        ...
        <Section
          ...
          icon={<Volume2 className={`w-4 h-4 ${isDarkTheme ? 'text-emerald-300' : 'text-emerald-600'}`} />}
        ...
        <Section title="HUD & Alerts" icon={<Sparkles className={`w-4 h-4 ${isDarkTheme ? 'text-amber-300' : 'text-amber-600'}`} />} description="Яркость и подсказки" isDarkTheme={isDarkTheme}>
          <div className="space-y-4">
            <div>
