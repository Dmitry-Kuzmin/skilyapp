...
import { LogIn, X, CheckCircle2, Hash, Sparkles } from 'lucide-react';
...
export function DuelJoinModal({ open, onClose, onDuelJoined }: DuelJoinModalProps) {
  ...
  const { showDuelJoinError, showDuelJoinSuccess, ToastContainer } = useSkilyAIToast();
                        ...
                        <div className="text-center mt-4 space-y-2">
                          <div className="flex justify-center gap-2">
                            {Array.from({ length: 4 }).map((_, i) => (
                              <motion.div
                                key={i}
                                className={`w-2.5 h-2.5 rounded-full transition-colors ${i < code.length
                                  ? 'bg-primary'
                                  : 'bg-muted'
                                  }`}
