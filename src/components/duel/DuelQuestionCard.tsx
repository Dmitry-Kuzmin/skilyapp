  ...
  const cardContent = (
    <div className="bg-card/95 backdrop-blur-sm border border-border rounded-3xl p-4 md:p-6 lg:p-8 shadow-2xl flex-1 flex flex-col overflow-y-auto overscroll-contain" style={{ WebkitOverflowScrolling: 'touch' }}>
      ...
      <div className={`flex flex-col md:flex-row gap-6 md:gap-8 h-full ${!hasImage ? 'justify-center' : ''}`}>
        ...
        {hasImage && (
          <div className="md:w-1/2 flex flex-col justify-center">
            <motion.div
              ...
              className="relative rounded-2xl overflow-hidden bg-black/50 aspect-video md:aspect-[4/3] w-full flex items-center justify-center group"
            >
              {/* Blur Background */}
              <div
        ...
        <div className={`flex flex-col justify-center ${hasImage ? 'md:w-1/2' : 'w-full max-w-2xl mx-auto'}`}>
          {/* Question Text */}
            ...
            {answerOptions.map((option, idx) => {
              ...
              const buttonElement = (
                <motion.button
                  ...
                  className={`p-4 rounded-xl border-2 text-left transition-all font-semibold text-base leading-snug relative overflow-hidden min-h-[60px] w-full select-none ${showResult
                    ? isCorrect
                ...
                >
                  {showResult && (isCorrect || isSelected) && (
                    <motion.div
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className={`absolute top-1/2 -translate-y-1/2 right-3 w-6 h-6 rounded-full flex items-center justify-center font-bold text-white text-xs ${isCorrect ? 'bg-green-500' : 'bg-red-500'}`}
                    >
                      {isCorrect ? '✓' : '✗'}
                    </motion.div>
                  )}
                  <span className={`block ${showResult ? 'pr-8' : ''}`}>
