      ...
      >
        ...
        {isUnlocked ? (
          <div className="space-y-2">
            {selectedBoost ? (
              ...
              >
                <div className="p-3 rounded-lg bg-indigo-500/10 border border-indigo-500/30">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-2xl">{selectedBoost.icon}</span>
                  ...
                  <Badge
                    ...
                    className={cn(
                      "text-xs",
                      selectedBoost.category === 'exploit' && "border-red-500/50 text-red-400",
                      selectedBoost.category === 'defense' && "border-blue-500/50 text-blue-400",
                      selectedBoost.category === 'utility' && "border-green-500/50 text-green-400"
                    )}
                  >
                    {selectedBoost.category === 'exploit' && 'Exploit'}
                    {selectedBoost.category === 'defense' && 'Defense'}
                    {selectedBoost.category === 'utility' && 'Utility'}
                  </Badge>
                </div>
                <button
                  onClick={() => setIsOpen(!isOpen)}
                  className="absolute top-1 right-1 w-6 h-6 rounded-full bg-zinc-800 hover:bg-zinc-700 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                >
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                className="w-full border-zinc-700 hover:border-indigo-500/50 text-zinc-400 hover:text-zinc-200"
              >
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            className="w-full border-zinc-700 hover:border-indigo-500/50"
          >
            {isUnlocking ? (
              'Разблокировка...'
            ) : isPremium ? (
        ...
        <AnimatePresence>
          {isOpen && isUnlocked && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="absolute top-full left-0 right-0 mt-2 z-50 bg-zinc-900 border border-zinc-700 rounded-xl p-2 max-h-64 overflow-y-auto space-y-1"
            >
              <button
                onClick={() => {
                  onSelectBoost(null);
                ...
                className="w-full p-2 text-left text-xs text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 rounded-lg transition-colors"
              >
                Очистить слот
              </button>
              {availableBoosts.map((boost) => (
                <button
                  ...
                  className={cn(
                    "w-full p-2 text-left rounded-lg transition-colors flex items-center gap-2",
