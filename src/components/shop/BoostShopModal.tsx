...
export function BoostShopModal({ open, onOpenChange }: BoostShopModalProps) {
  ...
  useEffect(() => {
    if (showHistory && dialogRef.current) {
      const dialogElement = dialogRef.current.closest('[role="dialog"]') as HTMLElement;
      if (dialogElement) {
    ...
    return (
      <>
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
              <Popover open={showHistory} onOpenChange={setShowHistory}>
                ...
                <PopoverContent 
                  className="max-w-[calc(100vw-2rem)] sm:max-w-lg p-0" 
                  align="end" 
                  side="bottom"
        ...
        <div className="relative flex-1 flex flex-col overflow-hidden">
          ...
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)} className="w-full flex flex-col flex-1 overflow-hidden min-h-0">
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent modalType="shop" hideCloseButton className="overflow-hidden flex flex-col p-0 overflow-x-hidden max-h-[95vh]" ref={dialogRef}>
