...
export const CHANGELOG: ChangelogRelease[] = [
    {
        version: 'May 2026 · Week 3',
        date: '2026-05-14',
        badge: '✨',
        title: 'Профиль-карточка, история платежей и умный демо-тест',
        description: 'Обновлённый профиль в стиле Hero, вкладка Платежи в истории, локализованный попап регистрации и удобный репорт об ошибке.',
        isLatest: true,
        changes: [
            { text: 'Профиль переработан в стиле Hero-карточки — аватар, имя, уровень и статистика смотрятся цельно и красиво', category: 'design' },
            { text: 'История: новая вкладка «Платежи» рядом с монетами — все покупки и подписки теперь в одном месте', category: 'feature' },
            { text: 'Подписки: правильно отображается разница между отменённым пробным периодом и отменённой платной подпиской', category: 'fix' },
            { text: 'Демо-тест из испанского лендинга теперь открывается на испанском, из русского — на русском. Язык страницы передаётся в тест и результаты', category: 'fix' },
            { text: 'Попап регистрации в демо-тесте появляется после 3-го вопроса (было: после 2-го), переведён на es / ru / en, стал компактнее', category: 'design' },
            { text: 'Репорт об ошибке в вопросе: выбери категорию — готово, кнопка «Отправить» убрана. Быстро и без сбоев', category: 'fix' },
            { text: 'AI-чат: новая структура сообщений и улучшенный рендеринг ответов', category: 'design' },
            { text: 'Уведомления: обновлена панель и логика доставки', category: 'fix' },
        ],
    },
    {
        version: 'May 2026 · Week 2',
        date: '2026-05-12',
        badge: '📊',
        title: 'Честная статистика: вероятность сдачи DGT на основе байесовского расчёта',
        description: 'Новая формула показывает реальные шансы сдать экзамен (≥27 из 30) с учётом твоих ошибок, покрытия тем и тренда последних ответов.',
        isLatest: false,
        changes: [
            { text: 'Вероятность сдачи DGT рассчитывается через биномиальное распределение — честно показывает шансы сдать 27+ из 30 вопросов', category: 'feature' },
            { text: 'Байесовское сглаживание для малых выборок — первые 50 ответов не обвисают в 0%, считается умнее', category: 'feature' },
            { text: 'Последние 50 ответов влияют на вероятность — если вы улучшаетесь прямо сейчас, это видно сразу', category: 'feature' },
            { text: 'Штраф за слабые темы: если какой-то раздел <60%, вероятность не может быть выше 50% — мотивирует учить всё', category: 'feature' },
            { text: 'Новый показатель «Опыт» вместо старой «Уверенности» — показывает как много ты потренировался (логарифмическая шкала)', category: 'feature' },
            { text: 'Документация по готовности к экзамену: развёрнутое объяснение формулы, таблицы вероятностей, интерпретация статистики', category: 'feature' },
            { text: 'Данные по игровым сессиям очищены — баг со счётом как процент (неправильно сохранялся 100-150%) исправлен для будущих тестов', category: 'fix' },
            { text: 'Производительность дашборда улучшена: 4 API-запроса сжаты в 1, телеметрия открывается быстрее', category: 'performance' },
        ],
    },
    {
        version: 'May 2026',
        date: '2026-05-07',
        badge: '🎯',
        title: 'Центр тестирования: игровой статус вместо дашборда',
        description: 'Три умных индикатора вместо скучной статистики — сразу видно где ты и куда идти.',
        isLatest: false,
        changes: [
            { text: 'Серия дней (⚡) в заголовке — твой стрик всегда на виду, мотивирует не прерывать цепочку', category: 'feature' },
            { text: 'Готовность к экзамену (%) со своим кружком прогресса — один взгляд и ясно, насколько ты близко к сдаче', category: 'feature' },
            { text: 'Очередь ошибок (⚠️) — показывает только когда есть что исправить, иначе не мешает', category: 'feature' },
            { text: 'Подсказки при нажатии на каждый индикатор — объясняют, что цифра значит и что делать дальше', category: 'feature' },
            { text: 'Счётчик доступных вопросов (например «300 / 2157») переехал в ту же строку — шапка стала чище', category: 'design' },
        ],
    },
    {
        version: 'Apr 2026 · Week 4',
        date: '2026-04-28',
        badge: '🤖',
        title: 'Skily AI — отдельная страница и ежедневный квиз',
        description: 'Полноэкранный чат с AI, утренние вопросы в Telegram, ежедневные задания в Duel Pass.',
        changes: [
            { text: 'Skily AI получил собственную полноэкранную страницу — открывай чат без лишних кнопок', category: 'feature' },
            { text: 'Утренний вопрос в Telegram каждый день — коротко и по делу, чтобы не терять темп', category: 'feature' },
            { text: 'Напоминание о баллах лицензии для тех, кто пропустил пару дней — не дадим потерять прогресс', category: 'feature' },
            { text: 'Ежедневные задания в Duel Pass теперь переведены на русский, испанский и английский', category: 'design' },
            { text: 'AI-чат в дашборде стал умнее: на мобильном открывается на полный экран, на десктопе — разворачивается рядом', category: 'design' },
        ],
    },
    {
        version: 'Apr 2026 · Week 2',
        date: '2026-04-14',
        badge: '🃏',
        title: 'Банк ошибок, новая игра и праздник уровня',
        description: 'Отдельный тренажёр для ошибок, карточки дорожных правил, Duolingo-стиль при повышении уровня.',
        changes: [
            { text: 'Банк ошибок полностью переработан — умная сортировка «самые сложные», «недавние», «по теме», прогресс исчезает когда вопрос освоен', category: 'feature' },
            { text: 'Новая игра «Правила дороги» — флэш-карточки по дорожным ситуациям, меняй между сторонами одним свайпом', category: 'feature' },
            { text: 'Анимация повышения уровня в стиле Duolingo — конфетти, карточка с наградой и кнопка «Забрать»', category: 'feature' },
            { text: 'Праздник после теста теперь с конфетти и звуком — финиш ощущается как победа', category: 'design' },
            { text: 'Прогресс по темам: видно сколько вопросов освоено в каждом разделе прямо в списке тем', category: 'feature' },
            { text: 'Блог обновлён — категории, мультиязычные статьи, статья о DGT 2026', category: 'feature' },
        ],
    },
    {
        version: 'Mar 2026',
