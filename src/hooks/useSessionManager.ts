 ...
 * Обеспечивает только 1 активную сессию одновременно (защита от sharing аккаунтов).
 *
 * ⚠️  ОТКЛЮЧЁН в AppProviders.tsx — причина:
 *   Хук проверяет session_token при каждом `window focus` событии.
 *   Когда пользователь открывает приложение в нескольких вкладках/браузерах,
 *   каждая вкладка создаёт свой токен → при переключении между ними
 *   одна вкладка инвалидирует токен другой → страница перезагружается / редирект на /.
 *
 * 🔍  Известные проблемы в текущей реализации:
 *
 *  1. Одна вкладка = одна сессия: хук не различает "другой браузер" от "другая вкладка
 *     того же браузера". BroadcastChannel / SharedWorker позволил бы синхронизировать
 *     токен между вкладками одного браузера без обращения к Supabase.
 *
 *  2. Токен привязан к device fingerprint, но fingerprint у одного устройства
 *     в разных браузерах разный → разные браузеры → разные токены → конфликт.
 *
 *  3. checkSession() на каждый `focus` слишком агрессивно: лучше проверять только
 *     если вкладка была неактивна > N минут (хранить `lastCheckedAt`).
 *
 *  4. manage-session Edge Function завершает все предыдущие сессии при создании новой,
 *     включая другие вкладки — это и есть источник перезагрузок.
 *
 * ✅  Как правильно включить обратно (когда появятся платящие пользователи):
 *
 *  Option A — минимально: добавить BroadcastChannel для синхронизации токена
 *    между вкладками одного браузера (не создавать новую сессию если токен уже есть).
 *
 *  Option B — правильно: использовать Supabase Realtime Presence вместо polling.
 *    Каждая вкладка публикует свой presence с channel_id = profileId.
 *    При появлении > 1 уникального device_fingerprint — показывать предупреждение
 *    или кикать самую старую сессию. Нет polling, нет конфликтов между вкладками.
 *
 *  Option C — по-простому: включать защиту только для isPremium === true.
 *    Бесплатные пользователи могут открывать сколько угодно вкладок,
 *    для них сессия не проверяется.
 */
