...
DGT усиливает блоки про восприятие риска и цифровые навыки. Теперь важно не просто помнить правило, а быстрее анализировать ситуацию.

### Что именно обновилось

| Что теперь спрашивают                         | Как тренироваться в Skilyapp                            |
|----------------------------------------------|---------------------------------------------------------|
| Сценарии с фото/видео (покадровый риск)       | Фильтр *«Новые вопросы 2025»* + игра **RaceGame**        |
| Комбинированные ответы (несколько подпунктов) | Дуэли на выбранную тему — тренируют скорость чтения     |
| Ассистенты водителя и электроника             | Карточки с AI-пояснениями + словарь терминов            |
| Цифровые сервисы DGT, cita previa, VeriFactu  | Чек-листы в разделе «Документы» + push-напоминания      |

### Мини-план на неделю
1. **Понедельник:** адаптивный тест, чтобы увидеть новые форматы.
2. **Вторник:** 3 сессии GuessTheSign + повтор знаков цифровых зон.
3. **Среда:** дуэли на темы «ассистенты» и «полосы BUS-VAO».
4. **Четверг:** RaceGame + разбор ошибок с AI.
5. **Пятница:** спокойная симуляция с таймером и анализ отчёта.

## CTA
Все новые типы вопросов появляются в Skilyapp сразу после обновлений базы DGT: мы добавляем сценарии, игры и AI-комментарии. Скачайте приложение и тренируйтесь по версии экзамена 2025 года.
  `,
},
...
Цифры в профиле — это подсказки, куда направить время. Вот как их использовать.

## Ключевые метрики и что с ними делать

| Метрика                        | Что говорит                                           | Действие в Skilyapp                              |
|--------------------------------|-------------------------------------------------------|--------------------------------------------------|
| **Точность по темам < 70%**    | Знания размыты                                        | Запустите быстрый тест только по этой теме       |
| **Streak в модуле прерван**    | Давно не повторяли, информация «остыла»               | Включите микротренировки и push-напоминание      |
| **Опасные ситуации** (красные) | Вы поздно замечаете риск                              | Пройдите RaceGame + сохраните вопросы в избранное |
| **Темп ответа падает**         | Слишком долго думаете над формулировками              | Используйте дуэли и режим «Спринт 20 вопросов»   |

## Алгоритм закрытия слабой темы
1. Найдите тему с низкой точностью в аналитике.
2. Пройдите короткий тест по ней, все ошибки сохраните.
3. Сформируйте «пакет повторения»: заметка + объяснение AI + карточка в словаре.
4. На следующий день сыграйте дуэль на 3–4 раунда и убедитесь, что мозг реагирует быстрее.

## План недели
- Понедельник: анализ отчёта и постановка целей.
- Вторник–четверг: микротренировки и игры по выбранным темам.
- Пятница: симуляция + сравнение новых данных со старыми.

## CTA
Skilyapp не просто считает ответы — он подсказывает, что делать дальше. Откройте аналитику, следуйте рекомендациям и тратьте время только на то, что реально подтягивает результат.
  `,
},
...
Мысли на родном языке — не помеха, если построить мост к испанскому.

## Инструменты Skilyapp
- Словарь терминов с переводами и примерами.
- Переключаемый интерфейс: вопросы на испанском, пояснения на русском.
- AI-помощник, который мгновенно переводит и проговаривает сложные обороты.

## Дневной ритуал
1. Повторите 5–7 терминов из словаря.
2. Пройдите мини-тест на испанском, но после каждого вопроса просите AI пересказать решение на русском.
3. Закрепите дуэлью или GuessTheSign, чтобы скорость чтения росла.

## Перед экзаменом
- Переключите весь интерфейс на испанский.
- Пройдите минимум две симуляции без перевода.
- Составьте мини-список слов, которые всё ещё вызывают вопросы, и прогоните их с AI-помощником.

## CTA
Skilyapp позволяет тренироваться на русском и испанском одновременно. Пользуйтесь гибкостью, чтобы уверенно читать формулировки на экзамене и не терять смысл.
    `,
  },
...
Эти символы похожи на привычные, но значение другое.

1. **R-305 Prohibido cambiar de sentido** — запрет разворота на всём участке.
2. **P-23 Ceda el paso a ciclistas** — уступайте велосипедистам всегда, даже если вы на главной.
3. **S-19 Fin de autopista** — конец автострады и особых правил.
4. **S-990 Zona de bajas emisiones** — экологическая зона с ограничениями по въезду.
5. **S-860 Carril BUS-VAO** — полоса для автобусов и автомобилей с несколькими пассажирами.

## Как тренироваться
- В GuessTheSign повторяйте знак до streak 15 — память становится «мышечной».
- Добавьте в словарь Skilyapp короткие заметки, чтобы мозг связывал образ с действием.
- Используйте аудио-режим и проговаривайте название вслух.

## CTA
В Skilyapp есть тематические подборки, аудио-карточки и мини-игры. Пять минут в день — и знаки перестают быть ловушками.
    `,
  },
...
Мотивация держится на ощущении прогресса. Геймификация в Skilyapp создаёт его каждый день.

## Что включить
- Streak и ежедневный бонус — формируют привычку.
- Дуэли и рейтинг — добавляют драйв.
- Сезонные задания — дают награды и монеты.

## Пример недели
- **Пн** — адаптивный тест + постановка цели.
- **Вт** — дуэль и разбор ошибок.
- **Ср** — RaceGame для скорости.
- **Чт** — словарь и карточки.
- **Пт** — симуляция с таймером.
- **Сб** — событие «Спринт» или командная дуэль.
- **Вс** — отдых + просмотр аналитики.

## CTA
Skilyapp начисляет монеты за дисциплину, напоминает о streak и показывает место в рейтинге. Добавьте игру в свою подготовку — и темп станет стабильным на любые 60 дней.
    `,
  },
