...
serve(async (req) => {
  ...
  try {
    ...
    if (is_premium) {
      ...
      const premiumResult = await processReward(rewardRow.premium_reward as RewardPayload, true, "error");
      ...
      const freeResult = await processReward(rewardRow.free_reward as RewardPayload, false, "error");
