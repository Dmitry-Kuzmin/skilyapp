...
-- ======================================
-- Версия 2.0 - Максимальная оптимизация
-- Возвращает ВСЕ данные включая topics, partners, premium
...
COMMENT ON INDEX idx_partners_user_id IS 'Ускоряет проверку партнерского статуса в Super RPC';
COMMENT ON INDEX idx_daily_tasks_user_date IS 'Ускоряет загрузку дейли заданий в Super RPC';
COMMENT ON INDEX idx_achievements_user_created IS 'Ускоряет загрузку достижений в Super RPC';
COMMENT ON INDEX idx_game_sessions_user_type IS 'Ускоряет подсчет статистики тестов в Super RPC';
