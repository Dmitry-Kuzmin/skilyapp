-- Update get_partner_dashboard function to include link activations stats
-- This migration updates the function to return statistics about partner link activations
...
BEGIN
  ...
  -- Extract partner_id and partner_code from JSONB
  ...
  -- Get link activation stats if partner has partner_code
  ...
  ELSE
    -- Initialize empty stats if no partner_code
    v_link_stats := ROW(0, 0, 0)::RECORD;
  END IF;
  
  -- Get stats (include both keys and link activations)
