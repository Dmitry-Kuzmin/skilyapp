...
CREATE TABLE IF NOT EXISTS public.duel_pass_season_rewards (
  ...
  sp_required INTEGER NOT NULL, -- Накопительное значение SP для достижения уровня
  free_reward JSONB, -- NULL если нет бесплатной награды на этом уровне
...
CREATE TABLE IF NOT EXISTS public.user_season_progress (
  ...
  season_id INTEGER NOT NULL REFERENCES public.duel_pass_seasons(id) ON DELETE CASCADE,
  season_points INTEGER NOT NULL DEFAULT 0, -- SP (Season Points)
  level INTEGER NOT NULL DEFAULT 1,
  premium_pass_purchased BOOLEAN NOT NULL DEFAULT false,
  premium_pass_purchased_at TIMESTAMPTZ,
  levels_skipped INTEGER NOT NULL DEFAULT 0, -- Купленные пропуски уровней
  final_level INTEGER, -- Сохраняется в конце сезона
  final_sp INTEGER, -- Сохраняется в конце сезона
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
