-- Comprehensive fix for get_dashboard_super RPC
-- 1. Adds missing profile fields (first_name, last_name, username, photo_url) to return JSON
-- 2. Fixes 'correct_answers' column reference error
-- 3. Improves stats calculation to filter out empty sessions (total_questions > 0)
...
BEGIN
  ...
  -- 2. Статистика
  -- FIX: Filter out empty sessions (total_questions > 0) to avoid inflating test count with empty attempts
  -- FIX: Use score instead of correct_answers if correct_answers doesn't exist
  -- Note: We assume 'score' holds the number of correct answers as per game_sessions schema
  SELECT 
    COUNT(*) FILTER (WHERE total_questions > 0) as tests_count, 
  ...
  SELECT json_build_object(
    ...
    'stats', json_build_object('tests_completed', COALESCE(v_stats.tests_count, 0), 'accuracy', COALESCE(v_stats.accuracy, 0)),
