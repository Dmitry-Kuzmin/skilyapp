-- Migration: Fix Duel Insurance and XP (v2 - Better Bot Handling)
-- 1. Update the payout function to handle insurance and XP rewards for losers
    ...
    FOR player IN SELECT * FROM duel_players WHERE duel_id = NEW.id LOOP
        ...
        ELSE
            ...
            IF has_insurance AND v_bet_amount > 0 THEN
                xp_reward := 15;
                refund_amount := ROUND(v_bet_amount * COALESCE(player.insurance_coverage_rate, 0.7)); 
