-------------------------------------------------------------------------
-- 1. ОСНОВНАЯ ЛОГИКА НАЧИСЛЕНИЯ/СПИСАНИЯ БАЛЛОВ (ДО 15, МИНИМУМ 0)
-------------------------------------------------------------------------
...
BEGIN
    ...
    CASE p_event_type
        WHEN 'daily_login' THEN 
            -- Если сегодня еще не получали, даем +1
        ...
        WHEN 'inactivity_decay' THEN v_points_delta := -1; -- Штраф за пропуск
    ...
    -- ФУНДАМЕНТАЛЬНЫЙ ЛИМИТ: строго от 0 до 15 баллов
    v_new_points := GREATEST(0, LEAST(15, v_new_points));

    -- Отмечаем, что сегодня бал за ежедневный логин уже был учтен
    ...
    RETURN jsonb_build_object('old_points', v_current_points, 'new_points', v_new_points, 'delta', v_points_delta, 'event', p_event_type);
...
-------------------------------------------------------------------------
-- 2. ВНЕДРЯЕМ ПРОВЕРКУ > 48 ЧАСОВ В ФУНКЦИЮ DASHBOARD
-------------------------------------------------------------------------
...
DECLARE
  ...
  v_result JSON;
  -- Переменные для контроля баллов
  v_last_decay DATE;
  v_days_missed INTEGER;
  v_last_active DATE;
BEGIN
  --------------------------------------------------
  -- АВТОМАТИЗАЦИЯ БАЛЛОВ: ПРОВЕРКА ПРОПУСКОВ 48ч
  --------------------------------------------------
  ...
  END IF;
  IF v_last_active IS NULL THEN v_last_active := CURRENT_DATE; END IF;

  -- 48 часов простоя означает, что разница в датах >= 2 дня
  IF (CURRENT_DATE - v_last_active) >= 2 AND (v_last_decay IS NULL OR v_last_decay < CURRENT_DATE) THEN
      ...
      -- максимум 3 балла штрафа за раз
  ...
  -- Автоматический плюс +1 балл за вход сегодня (сработает только раз в день)
  ...
  IF v_profile.id IS NULL THEN RETURN json_build_object('error', 'Profile not found'); END IF;
  ...
  -- ИСПРАВЛЕНИЕ ОШИБКИ ("v_daily_bonus has no field id"):
  -- Теперь v_daily_bonus - это сразу JSON
  ...
  IF v_daily_bonus IS NULL THEN 
      v_daily_bonus := json_build_object(
          'id', NULL,
          'current_streak', 0,
  ...
  IF v_active_season.id IS NOT NULL THEN SELECT * INTO v_season_progress FROM user_season_progress usp WHERE usp.user_id = p_user_id AND usp.season_id = v_active_season.id LIMIT 1; END IF;
  ...
  SELECT json_build_object(
    ...
    'active_season', CASE WHEN v_active_season.id IS NOT NULL THEN json_build_object('id', v_active_season.id, 'name_ru', v_active_season.name_ru, 'days_remaining', v_active_season.days_remaining) ELSE NULL END,
