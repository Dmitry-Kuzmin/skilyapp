-------------------------------------------------------------------------
-- 1. УСИЛЕНИЕ РОБАСТНОСТИ process_license_event
-------------------------------------------------------------------------
...
BEGIN
    ...
    IF v_current_points IS NULL THEN
        -- Если профиль найден, но баллы NULL (не должно быть, но на всякий случай)
        v_current_points := 8; -- дефолт
    END IF;
    ...
    IF p_event_type = 'daily_login' AND v_points_delta > 0 THEN
    ...
    RETURN jsonb_build_object('success', true, 'old_points', v_current_points, 'new_points', v_new_points, 'delta', v_points_delta);
EXCEPTION WHEN OTHERS THEN
    RAISE WARNING '[process_license_event] failed for user %: %', p_user_id, SQLERRM;
    RETURN jsonb_build_object('success', false, 'error', SQLERRM);
END;
$$;

-------------------------------------------------------------------------
-- 2. УСИЛЕНИЕ РОБАСТНОСТИ get_dashboard_super_v2 (ФИКС 500 ОШИБОК)
-------------------------------------------------------------------------
...
DECLARE
  ...
  v_last_active DATE;
BEGIN
  -- 1. Безопасная проверка и обновление баллов (в блоке EXCEPTION чтобы не уронить весь RPC)
  BEGIN
    ...
    -- Автоматический +1 балл за вход
    PERFORM process_license_event(p_user_id, 'daily_login');
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING '[get_dashboard_super_v2] Points update failed for user %: %', p_user_id, SQLERRM;
  END;

  -- 2. Получение профиля (Критическая секция)
  ...
  IF v_profile.id IS NULL THEN 
    RETURN json_build_object('error', 'Profile not found', 'p_user_id', p_user_id); 
  END IF;

  -- 3. Статистика (с защитой от пустых данных)
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING '[get_dashboard_super_v2] Stats calculation failed: %', SQLERRM;
    v_stats := NULL;
  END;
  ...
  INTO v_daily_bonus 
  FROM user_daily_bonus udb WHERE udb.user_id = p_user_id LIMIT 1;
  ...
  -- 6. Премиум, партнерка, сезоны (безопасные блоки)
  BEGIN
  ...
  IF v_active_season IS NOT NULL AND v_active_season.id IS NOT NULL THEN 
    BEGIN
      SELECT * INTO v_season_progress FROM user_season_progress usp WHERE usp.user_id = p_user_id AND usp.season_id = v_active_season.id LIMIT 1; 
    EXCEPTION WHEN OTHERS THEN v_season_progress := NULL; END;
  END IF;

  -- 7. Итоговая сборка JSON
  SELECT json_build_object(
    ...
    'active_season', CASE WHEN v_active_season IS NOT NULL AND v_active_season.id IS NOT NULL THEN json_build_object('id', v_active_season.id, 'name_ru', v_active_season.name_ru, 'days_remaining', v_active_season.days_remaining) ELSE NULL END,
    'season_progress', CASE WHEN v_season_progress IS NOT NULL AND v_season_progress.id IS NOT NULL THEN json_build_object('id', v_season_progress.id, 'level', v_season_progress.level, 'season_points', v_season_progress.season_points) ELSE NULL END,
