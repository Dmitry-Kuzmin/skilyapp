-- Migration: Fix License Points Activity Tracking and Audit (V2 - with Triggers)
-- Description: Ensures activity is tracked correctly via triggers on game sessions and includes a detailed audit log.

-- 1. Create audit table for detailed tracking
CREATE TABLE IF NOT EXISTS public.user_license_points_audit (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
    old_points SMALLINT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_license_points_audit_user ON public.user_license_points_audit(user_id, created_at DESC);

-- 2. Function to update activity date on any game session
CREATE OR REPLACE FUNCTION public.on_game_session_activity()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.user_id IS NOT NULL THEN
        UPDATE public.profiles 
        SET last_daily_point_at = CURRENT_DATE 
        WHERE id = NEW.user_id 
          AND (last_daily_point_at IS NULL OR last_daily_point_at < CURRENT_DATE);
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Trigger on game_sessions to mark user as active
DROP TRIGGER IF EXISTS tr_game_session_activity ON public.game_sessions;
CREATE TRIGGER tr_game_session_activity
AFTER INSERT ON public.game_sessions
FOR EACH ROW
EXECUTE FUNCTION public.on_game_session_activity();

-- 3. Rewrite process_license_event to be bulletproof
...
BEGIN
    ...
    INSERT INTO public.user_license_points_audit (user_id, old_points, new_points, delta, event_type)
    VALUES (p_user_id, v_current_points, v_new_points, v_points_delta, p_event_type);
...
-- 4. Fix get_dashboard_super_v2 (ensure search_path and robust decay)
...
BEGIN
  ...
  -- 1. MAIN DATA (Safe JSON population)
  ...
  RETURN json_build_object(
    ...
    'license_audit', (SELECT COALESCE(json_agg(a), '[]'::json) FROM (SELECT delta, event_type, created_at FROM public.user_license_points_audit WHERE user_id = p_user_id ORDER BY created_at DESC LIMIT 5) a),
...
-- 5. HEALING: Restore points for users penalized today wrongly
-- Apply to Dima's user and any other user who got decay today but had activity yesterday
-- User 2b6e3b89-8699-498f-9275-065f69781912 should be 11.
UPDATE public.profiles 
SET license_points = 11,
    last_daily_point_at = CURRENT_DATE
WHERE id = '2b6e3b89-8699-498f-9275-065f69781912';

-- Add entry to audit explaining the restore
INSERT INTO public.user_license_points_audit (user_id, old_points, new_points, delta, event_type, reason)
VALUES ('2b6e3b89-8699-498f-9275-065f69781912', 8, 11, 3, 'admin_restore', 'Restoring points after false inactivity decay bug');
