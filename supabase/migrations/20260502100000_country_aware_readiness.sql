-- Country-aware exam readiness: Spain uses topics/questions, Russia uses PDD tickets
...
BEGIN
  ...
  -- Профиль (включая preferred_country для country-aware расчётов)
  SELECT id, rank, xp, coins, boosts, streak_days, settings, subscription_status,
         ...
         COALESCE(LOWER(preferred_country), 'spain') as preferred_country
  INTO v_profile FROM public.profiles WHERE id = p_user_id;

  v_country := v_profile.preferred_country;

  -- Статистика и readiness — считаем по-разному для России и Испании
  IF v_country = 'russia' THEN

    -- Россия: статистика по билетам ПДД
    SELECT
      COUNT(DISTINCT ticket_id)::INTEGER as tests_completed,
      COALESCE(SUM(total_questions), 0)::INTEGER as total_questions,
      COALESCE(ROUND(SUM(best_score::NUMERIC / 100.0 * COALESCE(total_questions, 20))), 0)::INTEGER as correct_answers,
      COALESCE(ROUND(AVG(CASE WHEN best_score > 0 THEN best_score::NUMERIC END), 1), 0) as accuracy
    INTO v_stats
    FROM public.user_pdd_ticket_progress
    WHERE user_id = p_user_id AND country = 'ru';

    -- Россия: readiness по прогрессу по билетам (всего 40 билетов × 20 вопросов)
    SELECT
      ROUND(
        (COUNT(DISTINCT ticket_id)::NUMERIC / 40.0) * 100, 1
      ) as topics_covered_percent,
      COALESCE(SUM(total_questions), 0)::INTEGER as unique_questions_answered,
      COUNT(DISTINCT ticket_id)::INTEGER as topics_with_answers
    INTO v_readiness
    FROM public.user_pdd_ticket_progress
    WHERE user_id = p_user_id AND country = 'ru';
  ...
  ELSE
    ...
    -- Испания: статистика по game_sessions
    WITH s AS (
      SELECT COUNT(*) as cnt,
    ...
    -- Испания: readiness по темам и уникальным вопросам
    WITH tp AS (
