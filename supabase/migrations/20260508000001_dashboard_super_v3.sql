-- get_dashboard_super_v3
-- Single round-trip dashboard: replaces v2 + 3 client-side queries (duels, mistakes,
-- recent activity). Honest accuracy from user_progress (per-question source of truth)
-- instead of game_sessions.score (which had been written as a percentage by buggy
-- client writers — see backfill migration 20260508000000).
...
-- Index used by the recent-50 lookup
CREATE INDEX IF NOT EXISTS idx_user_progress_recent
  ON public.user_progress (user_id, last_attempt_at DESC NULLS LAST)
