-- =============================================================================
-- Migration: Fix duel payout trigger missing SET search_path
-- 2026-05-10: SECURITY DEFINER без search_path → функция выполняется с пустым
--             search_path и не находит таблицы (profiles, duel_players,
--             duel_transactions). Триггер падает молча → монеты/XP не
--             начисляются победителю, страховка не возвращается.
--             Тело функции совпадает с 20260228183500 — добавлено только
--             SET search_path = public. Триггер пересоздаётся явно для
--             надёжности (CREATE OR REPLACE FUNCTION его не трогает).
-- =============================================================================
    ...
    FOR player IN SELECT * FROM duel_players WHERE duel_id = NEW.id LOOP
        ...
        ELSE
            ...
            IF has_insurance AND v_bet_amount > 0 THEN
                xp_reward := 15;
                -- Используем константные 60% возврата (0.6)
