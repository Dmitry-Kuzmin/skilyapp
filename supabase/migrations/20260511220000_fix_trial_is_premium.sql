-- Fix: trial users with expired trial_until were still showing is_premium=true
-- because subscription_expires_at IS NULL was always evaluated as TRUE.
-- Now is_premium for 'trial' status correctly checks trial_until > NOW().
...
BEGIN
  ...
  -- 9. Premium / partner
  -- FIX: trial is only active if trial_until > NOW().
  -- Previously subscription_expires_at IS NULL was always TRUE for trial users → they appeared premium forever.
  SELECT subscription_status, subscription_expires_at, trial_until,
         (
           -- paid plans (monthly/annual) with explicit expiry
           (subscription_status IN ('active', 'pro') AND (subscription_expires_at > NOW() OR subscription_expires_at IS NULL))
           -- trial: only active while trial_until is in the future
