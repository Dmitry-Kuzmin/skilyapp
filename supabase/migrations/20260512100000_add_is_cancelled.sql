-- Add is_cancelled to premium JSON in get_dashboard_super_v3.
-- is_cancelled = true when the most recent premium Paddle purchase is cancelled
-- (subscription cancelled, but access still active until subscription_expires_at).
-- This lets the UI show "Cancelled, access until X" without a cancel button.
