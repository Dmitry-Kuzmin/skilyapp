-- New quest types: signs, mistakes_fixed, marathon_mode, streak_days
-- + auto-complete streak_days quests at assignment time

-- ── 1. Add new quest definitions ─────────────────────────────────────────────
...
VALUES
  ('sign_reader',     'signs',         'easy',   5,   25,  true, 'Знаток знаков',   'Ответь правильно на 5 вопросов про знаки'),
  ('sign_hunter',     'signs',         'medium', 15,  75,  true, 'Охотник знаков',  '15 правильных ответов на вопросы про знаки'),
  ('mistake_slayer',  'mistakes_fixed','medium', 5,   80,  true, 'Истребитель',     'Исправь 5 ошибок в режиме «Банк ошибок»'),
  ('error_cleaner',   'mistakes_fixed','hard',   15,  125, true, 'Чистильщик',      'Исправь 15 ошибок в режиме «Банк ошибок»'),
  ('marathon_run',    'marathon_mode', 'easy',   1,   30,  true, 'Марафонец',       'Пройди режим Марафон'),
  ('streak_warrior',  'streak_days',   'medium', 3,   60,  true, 'Постоянство',     'Заходи в приложение 3 дня подряд')
ON CONFLICT (id) DO NOTHING;

-- ── 2. Rewrite get_or_assign_daily_quests to auto-complete streak_days quests ─
...
BEGIN
  ...
  SELECT id, COALESCE(track_intensity, 'standard'), COALESCE(streak_days, 0)
    INTO v_user_id, v_intensity, v_streak
  ...
  -- Auto-complete streak_days quests if user already has qualifying streak
  UPDATE public.user_daily_quests udq
  SET
    current_progress = dq.target_value,
    is_completed     = true,
    completed_at     = COALESCE(udq.completed_at, now())
  FROM public.daily_quest_definitions dq
  WHERE udq.quest_id    = dq.id
    AND udq.user_id     = v_user_id
    AND udq.assigned_at = v_today
    AND dq.category     = 'streak_days'
    AND v_streak        >= dq.target_value
